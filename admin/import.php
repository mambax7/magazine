<?php
// $Id: import.php,v 1.6 2005/02/07 01:25:24 phppp Exp $
//  ------------------------------------------------------------------------ //
...
{
    include_once MAG_ROOT_PATH . '/include/mb_dummy.php';
} 
...
mag_admin_menu( _AM_MAG_ADMENUB );
mag_textinfo( _AM_MAG_IMPORT, _AM_MAG_IMPORTTEXT );
...
function proc_save()
{
    ...
    {
        echo "<font color=red>" . _AM_MAG_IMPORT_ERRDIREXI . "</font><br>$dir_src<br>\n";
        return;
    } 
    ...
    if ( $filter )
    {
        ...
        {
            echo "<font color=red>" . _AM_MAG_IMPORT_ERRFILEXI . "</font><br>$filter<br>\n";
            return;
        } 

        if ( !is_executable( $filter ) )
        {
            echo "<font color=red>" . _AM_MAG_IMPORT_ERRFILEXEC . "</font><br>$filter<br>\n";
            return;
        } 
    } 
    ...
    if ( $flag_image )
    {
        if ( !$flag_copy )
        {
            echo "<font color=red>" . _AM_MAG_IMPORT_ERRNOCOPY . "</font><br>\n";
            return;
        } 

        if ( !$dir_image )
        {
            echo "<font color=red>" . _AM_MAG_IMPORT_ERRNOIMGDIR . "</font><br>\n";
            return;
        } 

        if ( file_exists( $dir_image_full ) && !is_dir( $dir_image_full ) )
        {
            echo "<font color=red>" . _AM_MAG_IMPORT_ERRIMGDIREXI . "</font><br>$dir_image_full<br>\n";
            return;
        } 
    } 
    ...
    {
        echo "<hr>\n";
        echo "<a href=\"JavaScript:history.back()\">" . _AM_MAG_IMPORT . "</a>";
    } elseif ( $error_flag )
    {
        echo "<hr>\n";
        echo "<font color=red>Unsuccessful!!</font><br><br>\n";
        echo "<a href=\"allarticles.php\">" . _AM_MAG_ARTICLEMANAGE . "</a><br>";
        echo "<a href=\"JavaScript:history.back()\">" . _AM_MAG_IMPORT . "</a>";
    } 
    else
    {
        echo "<hr>\n";
        echo "<b>" . _AM_MAG_DBUPDATED . "</b><br><br>\n";
        echo "<a href=\"allarticles.php\">" . _AM_MAG_ARTICLEMANAGE . "</a>";
    } 
} 
...
{ 
    ...
    if ( !file_exists( $file ) )
    {
        echo "$file: <font color=red>" . _AM_MAG_IMPORT_ERRFILEEXI . "</font><br>\n";
        return;
    } 
    // .html .htm
...
{
    global $xoopsUser, $magConfig, $HTTP_POST_VARS;
...
{
    global $xoopsModule, $xoopsModuleConfig, $xoopsConfig, $HTTP_SERVER_VARS, $HTTP_POST_VARS, $magPathConfig;

    $sform = new XoopsThemeForm( _AM_MAG_IMPORT, "op", xoops_getenv( 'PHP_SELF' ) );

    $xt = new MagCategory();
    ob_start();
    echo $xt->makeSelBox( 0, 0, "categoryid" );
    $sform->addElement( new XoopsFormLabel( _AM_MAG_CATEGORY, ob_get_contents() ) );
    ob_end_clean();

    $dir = MAG_HTML_PATH;
    $sform->addElement( new XoopsFormText( _AM_MAG_IMPORT_DIRNAME, 'dir_src', 50, 255, $dir ), true );

    $sform->insertBreak( "<b>" . _AM_MAG_IMPORT_HTMLPROC . "</b>", "even" );
    $sform->addElement( new XoopsFormText( _AM_MAG_IMPORT_EXTFILTER, 'filter', 50, 255, '' ), false );
	...
	$allow_word = ( $allow_com['ini_set'] ) ? _AM_MAG_IMPORTWORDINYES : _AM_MAG_IMPORTWORDINNO;
	$allow_com_text = ( $allow_com['word'] == true ) ? _AM_MAG_IMPORTWORDYES : _AM_MAG_IMPORTWORDNO;
	
	$import_extra = "<br />
		<li><b>"._AM_MAG_IMPORTCOMENABLED."</b> $allow_com_text</li>
		<li><b>"._AM_MAG_IMPORTWORDINSTALL."</b> $allow_word</li>
		<li>"._AM_MAG_IMPORTWORDSELECT."</li>
		";

    mag_textinfo( _AM_MAG_IMPORTWORD, _AM_MAG_IMPORTWORDTXT.$import_extra );
    if ( $allow_com['ini_set'] && $allow_com['word'] )
    {
        ...
        echo "<th colspan = '2'>" . _AM_MAG_IMPORTWORD . "</th>";
        echo "<tr>";
        echo "<td class = 'head'>" . _AM_MAG_EDITWORDBROWSE . "</td>";
        ...
        echo "<td class = 'even'>";
        echo "<input type='submit' name='op' class='formButton' value=". _SUBMIT. " />";
        echo "</td></tr>";
        echo "</form>";
        echo "</table>";
    }
    ...
    {
        echo "<b><font color='tomato'>" . _AM_MAG_WORDNOTINSTALLED . "</font></b><br /><br />";
    }

    ob_start();
    echo "<input type='checkbox' name='html' checked> ";
    echo _AM_MAG_IMPORT_BODY . "<br>\n";
    echo "<input type='checkbox' name='index' checked> ";
    echo _AM_MAG_IMPORT_INDEXHTML . "<br>\n";
    echo "<input type='checkbox' name='link' checked> ";
    echo _AM_MAG_IMPORT_LINK . "<br>\n";
    echo "<input type='checkbox' name='image' checked> ";
    echo _AM_MAG_IMPORT_IMAGE . "<br>\n";
    echo "<input type='checkbox' name='atmark' checked> ";
    echo _AM_MAG_IMPORT_ATMARK . "<br><br>\n";
    $sform->addElement( new XoopsFormLabel( '', ob_get_contents() ) );
    ob_end_clean();

    $sform->insertBreak( "<b>" . _AM_MAG_IMPORT_IMAGEPROC . "</b>", "even" );

    $dir = $magPathConfig['graphicspath'];
    $sform->addElement( new XoopsFormText( _AM_MAG_IMPORT_IMAGEDIR, 'dir_image', 50, 255, $dir ), false );

    $img_checkbox = new XoopsFormCheckBox( _AM_MAG_IMPORT_IMAGECOPY, "image_copy", 1 );
    $img_checkbox->addOption( 1, " " );
    $sform->addElement( $img_checkbox );

    $sform->insertBreak( "<b>" . _AM_MAG_IMPORT_TESTMODE . "</b><br >" . _AM_MAG_IMPORT_TESTDB, "even" );

    $test_checkbox = new XoopsFormCheckBox( _AM_MAG_IMPORT_TESTEXEC, "test", 1 );
    $test_checkbox->addOption( 1, " " );
    $sform->addElement( $test_checkbox );

    $texttest_checkbox = new XoopsFormCheckBox( _AM_MAG_IMPORT_TESTTEXT, "test_text", 1 );
    ...
    $butt_save = new XoopsFormButton( "", "", _AM_MAG_SAVE, "submit" );
