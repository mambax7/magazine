    ...
    case "upload":
        ...
        if ( $_FILES['uploadfile']['name'] != "" )
        {
            if ( file_exists( XOOPS_ROOT_PATH . "/" . _UPLOADS . "/" . $_FILES['uploadfile']['name'] ) )
                redirect_header( 'upload.php', 2, _AM_MAG_DOWN_IMAGEEXIST );
		...
		$tmppath= $uploader->getSavedFileName();
        $working_image = ( !empty($tmppath) ) ? $uploader->getSavedFileName() : $_POST['downfile'];//dqflyer fixed
...
//		redirect_header( "upload.php", 2, $working_image );
        /**
         * end
         */
        if ( $_POST['img_resize'] && !empty( $working_image ) )
        {
            $thumbnail = mag_createthumb( $working_image, _UPLOADS, _UPLOADS, $_POST['img_width'], $_POST['img_height'], $_POST['img_quality'], 1, $_POST['img_aspect'] ); //dqflyer fiexed
            @chmod( $thumbnail, 0777 );

            $img_path = XOOPS_ROOT_PATH . "/" . _UPLOADS . "/";
			if ( $_POST['img_del'] )
			{
	            $img_source = $working_image;
			}
			$img_thumb = basename($thumbnail);

            if ( file_exists( "{$img_path}{$img_thumb}" ) )
            {
                $error = 0;
//                if ( !@copy( "{$img_path}/thumbs/{$img_thumb}", "{$img_path}/{$working_image}" ) )
//                    $error = 1;
                if ( !@unlink( "{$img_path}{$img_source}" ) )
                    $error = 2;
                @touch( "{$img_path}{$img_thumb}" );
                @chmod( "{$img_path}{$img_thumb}", 0644 );
        ...
        if ( $_POST['use_imgman'] )
        {
            $image_mimetype = ( isset( $uploader->getMediaType ) ) ? $uploader->getMediaType() : mag_retmime( "{$img_path}/{$working_image}" );
    ...
    case "delfile":
        ...
        if ( isset( $confirm ) && $confirm == 1 )
        {
            ...
            if ( file_exists( $filetodelete ) )
            {
                ...
                if ( @unlink( $filetodelete ) )
                    redirect_header( 'upload.php', 1, _AM_MAG_DOWN_FILEDELETED );
                else
                    redirect_header( 'upload.php', 1, _AM_MAG_DOWN_FILEERRORDELETE );
            } 
            exit();
        } 
        else
        {
            ...
            {
                redirect_header( 'upload.php', 1, _AM_MAG_DOWN_NOFILEERROR );
                exit();
            } 
            xoops_cp_header();
            xoops_confirm( array( 'op' => 'delfile', 'uploadpath' => $_POST['uploadpath'], 'downfile' => $_POST['downfile'], 'confirm' => 1 ),
                'upload.php', _AM_MAG_DOWN_DELETEFILE . "<br /><br />" . $_POST['downfile'], _AM_MAG_BDELETE );
        } 
        break;

    case "default":
    default:
        include_once '../class/lists.php';
        ...
        $dirarray = array( 1 => $magPathConfig['sgraphicspath'], 2 => $magPathConfig['graphicspath'], 3 => $magPathConfig['logopath'], 4 => "/modules/" . MODDIR . "/images" );
        $namearray = array( 1 => _AM_MAG_DOWN_CATIMAGE , 2 => _AM_MAG_DOWN_SCREENSHOTS, 3 => _AM_MAG_DOWN_LOGOIMAGEDIR, 4 => _AM_MAG_DOWN_MAINIMAGEDIR );
        $listarray = array( 1 => _AM_MAG_DOWN_FCATIMAGE , 2 => _AM_MAG_DOWN_FSCREENSHOTS, 3 => _AM_MAG_DOWN_FLOGOIMAGEDIR, 4 => _AM_MAG_DOWN_FMAINIMAGEDIR );

        mag_admin_menu( _AM_MAG_MUPLOADS );
        if ( $rootpath > 0 )
            echo "<div><b>" . _AM_MAG_DOWN_FUPLOADPATH . "</b> " . XOOPS_ROOT_PATH . "/" . $dirarray[$rootpath] . "</div>\n
				  <div><b>" . _AM_MAG_DOWN_FUPLOADURL . "</b> " . XOOPS_URL . "/" . $dirarray[$rootpath] . "</div><br />\n";

        $pathlist = ( isset( $listarray[$rootpath] ) ) ? $namearray[$rootpath] : '';
        $namelist = ( isset( $listarray[$rootpath] ) ) ? $namearray[$rootpath] : '';

        $iform = new XoopsThemeForm( _AM_MAG_DOWN_FUPLOADIMAGETO . $pathlist, "op", xoops_getenv( 'PHP_SELF' ) );
        $iform->setExtra( 'enctype="multipart/form-data"' );

        ob_start();
        $iform->addElement( new XoopsFormHidden( 'dir', $rootpath ) );
        mag_getDirSelectOption( $namelist, $dirarray, $namearray );
        $iform->addElement( new XoopsFormLabel( _AM_MAG_DOWN_FOLDERSELECTION, ob_get_contents() ) );
        ob_end_clean();
        ...
        echo "<div><b>" . _AM_MAG_FREE_DISK_SPACE . ": </b>" . mag_myfilesize( $free_space ) . "</div>";
        echo "<div><b>" . _AM_MAG_TOTAL_USED_SPACE . ": </b>" . mag_myfilesize( $used_space ) . "</div>";
        echo "<div><b>" . _AM_MAG_TOTAL_DISK_SPACE . ": </b>" . mag_myfilesize( $total_space ) . "</div>";
        
		if ( $rootpath > 0 )
        {
            $graph_array = &MagLists::getListTypeAsArray( XOOPS_ROOT_PATH . "/" . $dirarray[$rootpath], $type = "images" );
            ...
            $indeximage_tray = new XoopsFormElementTray( _AM_MAG_DOWN_FSHOWSELECTEDIMAGE, '&nbsp;' );
            ...
            clearstatcache();
            $iform->addElement( new XoopsFormFile( _AM_MAG_DOWN_FUPLOADIMAGE, 'uploadfile', '' ), false );
            $iform->addElement( new XoopsFormText( _AM_MAG_IMAGEPREFIX, 'img_prefix', 15, 80, '' ), false );
            $iform->insertBreak( '', 'even' );
            $iform->addElement( new XoopsFormRadioYN( _AM_MAG_RESIZEIMAGE, 'img_resize', 0, ' ' . _AM_MAG_YES . '', ' ' . _AM_MAG_NO . '' ) );
            $iform->addElement( new XoopsFormRadioYN( _AM_MAG_DELIMAGE, 'img_del', 0, ' ' . _AM_MAG_YES . '', ' ' . _AM_MAG_NO . '' ) );
            $iform->insertBreak( _AM_MAG_IMGRESIZEOPTIONS, 'even' );
            $iform->addElement( new XoopsFormText( _AM_MAG_IMAGENEWWIDTH, 'img_width', 10, 80, '300' ), false );
            $iform->addElement( new XoopsFormText( _AM_MAG_IMAGENEWHEIGHT, 'img_height', 10, 80, '250' ), false );
            $iform->addElement( new XoopsFormText( _AM_MAG_IMAGENEWQUALITY, 'img_quality', 3, 30, '100' ), false );
            $iform->addElement( new XoopsFormRadioYN( _AM_MAG_IMAGEKEEPASPECT, 'img_aspect', 0, ' ' . _AM_MAG_YES . '', ' ' . _AM_MAG_NO . '' ) );
            ...
            {
                $iform->insertBreak( '', 'even' );
                $iform->insertBreak( _AM_MAG_XOOPSIMGMAN, 'bg3' );
                ...
                $iform->addElement( new XoopsFormRadioYN( _AM_MAG_USEIMAGEMANGER, 'use_imgman', 0, _YES, _NO ) );
            ...
            $butt_dup = new XoopsFormButton( '', '', _AM_MAG_BUPLOAD, 'submit' );
            $butt_dup->setExtra( 'onclick="this.form.elements.op.value=\'upload\'"' );
            $dup_tray->addElement( $butt_dup );

            $butt_dupct = new XoopsFormButton( '', '', _AM_MAG_BDELETEIMAGE, 'submit' );
