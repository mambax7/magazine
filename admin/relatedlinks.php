<?php
// $Id: relatedlinks.php,v 1.6 2005/02/07 01:25:25 phppp Exp $
//  ------------------------------------------------------------------------ //
...
switch ( $op )
{
    case "saverelated";
        ...
        if ( isset( $relatedlink['relatedlink_topicid'] ) )
        {
            for ( $i = 0; $i < count( $relatedlink['relatedlink_topicid'] ); $i++ )
            {
                ...
                if ( $related_link == 1 )
                {
                    $result = $xoopsDB->query( "SELECT * FROM " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " WHERE relatedlink_lid =" . $relatedlink['relatedlink_lid'][$i] . "" );
                    if ( !$xoopsDB->getRowsNum( $result ) )
                    {
                        $query = "INSERT INTO " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " (relatedlink_id, relatedlink_topicid, relatedlink_url, 
                    ...
                    {
                        $query = "UPDATE " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " set 
                ...
                {
                    $query = "DELETE FROM " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " WHERE relatedlink_mod =" . $relatedlink['relatedlink_mod'][$i] . " AND relatedlink_lid = " . $relatedlink['relatedlink_lid'][$i] . "";
                } 
            } 
        } 
        else
        {
            ...
            {
                $query = "UPDATE " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " 
            ...
            {
                $query = "INSERT INTO " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " (relatedlink_id, relatedlink_topicid, relatedlink_url, relatedlink_urlname, relatedlink_weight, relatedlink_mod) ";
        ...
        redirect_header( "relatedlinks.php", 1, _AM_MAG_RELATED_DBUPDATED );
        break;
    ...
    case "delete";
        ...
        if ( isset( $ok ) && $ok == 1 )
        {
            $sql = "DELETE FROM " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " WHERE relatedlink_id=" . $_POST['related'] . "";
            ...
            redirect_header( "relatedlinks.php", 1, _AM_MAG_RELATED_DELETED );
            exit();
        } 
        else
        {
            xoops_cp_header();
            xoops_confirm( array( 'op' => 'delete', 'related' => $_GET['related'], 'ok' => 1 ), 'relatedlinks.php', _AM_MAG_DELETERELEATEDLINK );
        } 
        break;
    ...
    case "addrelated":
        ...
        mag_admin_menu( _AM_MAG_RELATEDLINKS );
        mag_textinfo( _AM_MAG_RELATEDLINKLIST2, _AM_MAG_RELATEDLINKLISTTXT2 );

        $relatedlink_url = '';
        $relatedlink_urlname = '';
        $relatedlink_weight = 0;

        $sform = new XoopsThemeForm( _AM_MAG_ADDRELATEDLINK, "op", xoops_getenv( 'PHP_SELF' ) );
        if ( isset( $_GET['related'] ) )
        {
            $sql = "SELECT * FROM " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . " WHERE relatedlink_id=" . $_GET['related'] . "";
        ...
        $sform->addElement( new XoopsFormText( _AM_MAG_RELATED_URL, 'relatedlink_url', 50, 255, $relatedlink_url ), true );
        $sform->addElement( new XoopsFormText( _AM_MAG_RELATED_URLNAME, 'relatedlink_urlname', 50, 255, $relatedlink_urlname ), true );
        $sform->addElement( new XoopsFormText( _AM_MAG_RELATED_WEIGHT, 'relatedlink_weight', 4, 4, $relatedlink_weight ) );
        ...
        {
            echo "<h4>" . _AM_MAG_RELATEDLINKSLIST . "</h4>";
            ...
            echo "<td align='left' width = '50%'class='bg3'><b>" . _AM_MAG_RELATED_URL . "</b></td>";
            echo "<td align='center' width = '10%'class='bg3'><b>" . _AM_MAG_RELATED_WEIGHT . "</b></td>";
            echo "<td align='center' width = '17%' class='bg3'><b>" . _AM_MAG_ACTION . "</td>";
            echo "</tr>";

            $sql = "SELECT * FROM " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . "  WHERE relatedlink_topicid = " . $_GET['articleid'] . " ORDER BY relatedlink_id ";
            $result = $xoopsDB->query( $sql );
    ...
    case "addrelatedlink":
        ...
        mag_admin_menu( _AM_MAG_RELATEDLINKS );
        mag_textinfo( _AM_MAG_RELATEDNEWS, '' );
        ...
        while ( list( $lid, $title, $url ) = $xoopsDB->fetchrow( $result ) )
        {
            ...
            $sql = "SELECT * FROM " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . "  WHERE relatedlink_topicid = " . $_GET['articleid'] . " AND relatedlink_mod = 2 ORDER BY relatedlink_id ";
    ...
    default:
        ...
        xoops_cp_header();
        mag_admin_menu( _AM_MAG_RELATEDLINKS );
        mag_textinfo( _AM_MAG_RELATEDLINKLIST, _AM_MAG_RELATEDLINKLISTTXT );
        if ( $totalcount > 0 )
        {
            ...
            $sql = "SELECT * FROM " . $xoopsDB->prefix( MAG_RELATEDLINKS ) . "  ORDER BY relatedlink_id ";
            $result = $xoopsDB->query( $sql );
			$count = $xoopsDB->getRowsNum( $result );
