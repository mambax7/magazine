<?php
// $Id: spotlightblock.php,v 1.7 2005/02/07 01:25:25 phppp Exp $
//  ------------------------------------------------------------------------ //
...
switch ( $op )
{
    case 'submit':
        ...
        $sql = "UPDATE " . $xoopsDB->prefix( MAG_SPOTLIGHT )
         . " SET item='" . $item
         . "', image='" . $image
        ...
        if ( $result )
        {
            redirect_header( "spotlightblock.php", 1 , _AM_MAG_DBUPDATED );
        } 
        else
        {
            $error = _AM_MAG_ERROR_CREATESPOTLIGHT . $sql;
    ...
    default:
        ...
        xoops_cp_header();
        mag_admin_menu( _AM_MAG_DOCSPOTLIGHTHEADING );
        mag_textinfo( _AM_MAG_DOCSPOTLIGHTINFO, _AM_MAG_DOCSPOTLIGHTTEXT );

        $sql = ( "SELECT item, image, itemlength, imagewidth, imageheight, sum_type FROM " . $xoopsDB->prefix( MAG_SPOTLIGHT ) . " WHERE sid=1" );
        $result = $xoopsDB->query( $sql );
        ...
        {
            $item = MagArticle::getLastArticleByCategory();
            $currentspot_desc = _AM_MAG_USE_LASTPUBLISHED . " <b>No:</b> " . $item;
        } 

        $current_spot = new MagArticle( $item );
        echo "<h4>" . _AM_MAG_CURRENT_SPOT . "</h4>";
        if ( $current_spot )
        {
            echo "<table border='0' width='100%' cellpadding ='2' cellspacing='1' class = 'outer'>";
            echo "<tr><td class = \"bg3\" colspan = \"2\">" . _AM_MAG_STORYID . "</td></tr>";
            echo "<tr><td class = \"head\">" . _AM_MAG_STORYID . "</td><td class = \"even\">" . " <b>No:</b> " . $currentspot_desc;
            echo "</td></tr>";
            echo "<tr><td class = \"head\">" . _AM_MAG_TITLE . "</td><td class = \"even\">" . $current_spot->textLink();
            echo "</td></tr>";
            echo "<tr><td class = \"head\">" . _AM_MAG_AUTHOR . "</td><td class = \"even\">" . $current_spot->uname();
            echo "</td></tr>";
            echo "<tr><td class = \"head\">" . _AM_MAG_CATEGORYNAME . "</td><td class = \"even\">" . $current_spot->category->textLink();
            echo "</td></tr>";
            $published_date = ( $current_spot->published() > '0' ) ? formatTimestamp( $current_spot->published(), "$xoopsModuleConfig[timestamp]" ) : "" . _AM_MAG_NOTPUBLISHED . "";
            echo "<tr><td class = \"head\">" . _AM_MAG_PUBLISHED . "</td><td valign = 'top' class = \"even\">" . $published_date;
            echo "</td></tr>";

            echo "<tr><td valign = 'top' class = \"head\">" . _AM_MAG_DOCSPOTLIGHTIMAGE . "</td>";
            echo "<td>";
            if ( $indeximage )
            {
                $index_image = MAG_ARTICLEIMG_URL . '/' . $indeximage;
        ...
        $sform = new XoopsThemeForm( _AM_MAG_DOCSPOTLIGHTFORM , "op", xoops_getenv( 'PHP_SELF' ) );
        $image_option_tray = new XoopsFormElementTray( _AM_MAG_DOCSPOTLIGHTIMAGE, '<br />' );
        ...
        $indeximage_select->setExtra( "onchange='showImgSelected(\"image\", \"spot_image\", \"" . $magPathConfig['graphicspath'] . "\", \"\", \"" . XOOPS_URL . "\")'" );
        $indeximage_tray = new XoopsFormElementTray( '', '&nbsp;' );
        $indeximage_tray->addElement( $indeximage_select );
        if ( !empty( $indeximage ) )
        {
            $indeximage_tray->addElement( new XoopsFormLabel( '', "<div style='padding: 8px;'><img src='" . XOOPS_URL . "/" . $magPathConfig['graphicspath'] . "/" . $indeximage . "' height = '$imageheight' width = '$imagewidth' name='image' id='image' alt='' /></div>" ) );
        } 
        else
        {
        ...
        {
            $sform->addElement( new XoopsFormText( _AM_MAG_SPOTIMAGE_MAXWIDTH, 'imagewidth', 10, 80, intval( $imagewidth ) ), true );
            $sform->addElement( new XoopsFormText( _AM_MAG_SPOTIMAGE_MAXHEIGHT, 'imageheight', 10, 80, intval( $imageheight ) ), true );
            $sform->addElement( new XoopsFormText( _AM_MAG_SPOTDOCUMENT_MAXLENGTH, 'itemlength', 10, 80, intval( $itemlength ) ), true );
            $align_select = new XoopsFormSelect( _AM_MAG_SPOTDOCUMENT_SUMTYPE, "sum_type", $sum_type );
            $align_select->addOptionArray( array( 1 => _AM_MAG_SPOTDOCUMENT_SUBTITLE, 2 => _AM_MAG_SPOTDOCUMENT_SUMMARY , 3 => _AM_MAG_SPOTDOCUMENT_MAINTEXT ) );
        ...
        $spot_id_radio = new XoopsFormRadio( _AM_MAG_DOCSPOTLIGHTDOC, "spot_id", $currentspot_id );
        $spot_id_radio->addOption( 0, _AM_MAG_USE_LASTPUBLISHED . "; " . _AM_MAG_OTHERWISE_CHOOSEANARTICLE );
        $sform->addElement( $spot_id_radio );

        $start = isset( $_GET['start'] ) ? intval( $_GET['start'] ) : 0;
        $articlearray = MagArticle::getAllArticle( $xoopsModuleConfig['lastart'], $start, 'online', 0, 0, 'articleid DESC' );
        $totalcount = count( MagArticle::getAllArticle( 0, 0, 'published' ) );
        $scount = count( $articlearray );

        $heading = array( _AM_MAG_STORYID, _AM_MAG_TITLE, _AM_MAG_SECTION, _AM_MAG_POSTER, _AM_MAG_STATUS, _AM_MAG_PUBLISHED, _AM_MAG_SPOTIT );
        $article_table = "\n<tr><td colspan='2'>";
        ...
        if ( count( $articlearray ) == '0' )
            $article_table .= "\n<tr ><td align='center' colspan ='10' class = 'head'><b>" . _AM_MAG_NOARTICLEFOUND . "</b></td></tr>";

        for ( $i = 0; $i < count( $articlearray ); $i++ )
        {
            $allarticles['status'] = ( $articlearray[$i]->offline == '0' ) ? $online : $offline;
            $allarticles['published'] = ( $articlearray[$i]->published() > '0' ) ? formatTimestamp( $articlearray[$i]->published(), "$xoopsModuleConfig[timestamp]" ) : "" . _AM_MAG_NOTPUBLISHED . "";
            $allarticles['topic'] = $articlearray[$i]->category->textLink();
            $allarticles['topic'] = ( !empty( $allarticles['topic'] ) ) ? $allarticles['topic'] : _AM_MAG_NOSUCHSECTION;
