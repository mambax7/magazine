<?php
// $Id: articleres.php,v 1.7 2005/02/07 01:25:24 phppp Exp $
// ------------------------------------------------------------------------ //
...
if ( !$restore_id ) die( _AM_MAG_NORESTORE );
if ( !( $xoopsUser->isAdmin() ) ) die( _NOPERM );

$article = new MagArticleRes( $restore_id );
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td align="left" valign="top" width="75%">
			<div><b>' . _AM_MAG_AUTHOR . '</b> ' . $article->uname() . ' 
            <div><b>' . _AM_MAG_PUBLISHEDDATE . '</b> '.$datetime.'</div> 
			<div><b>' . _AM_MAG_RESTORE_VERSION . ':</b> '.$article->version.'</div>
			<div><b>' . _AM_MAG_ID . '</b> '.$article->articleid.'</div>
		</td>
      </tr>
    </table></td>
  </tr>
...
echo '<div><b>'.$article->subtitle().'</b></div><br /> 
		'.$maintext.'';
		echo '<br /><br /><a href="' . MAG_ROOT_URL . '/article.php?articleid=' . $article->articleid() . '">' . MAG_ROOT_URL . '/article.php?articleid=' . $article->articleid() . '</a><br /><br />';
