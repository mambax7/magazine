...
function mag_listGroups($grps = "-1")
{
    global $xoopsDB, $myts;
 ...
 * See docs for usage
 */
function Mag_checkAccess($grps, $time = -1, $message = '')
{
 ...
 * @return 
 */
function mag_checkadminAccess($grps)
{
    global $xoopsUser, $xoopsModule;
...
function mag_saveAccess($grps)
{
    if (is_array($grps)) $grps = implode(" ", $grps);
    return($grps);
}

function mag_getGroupIda($grps)
{
    $ret = array();
